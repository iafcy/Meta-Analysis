...
from prompt import generate_qa_prompt

def load_data_to_prompt():
    with open('./data/qa_test.json', 'r') as file:
        data = json.load(file)
    
    for meta_analysis in data:
        for x in meta_analysis['data']:
            x['prompt'] = generate_qa_prompt(x)
    
    return data

def predict_qa(model: Bot, output_dir: str):
    data = load_data_to_prompt()

    for meta_analysis in tqdm(data, desc='Prediction for Qaulity Assessment:'):
        start_time = timer()
    ...
    with open(f'{output_dir}/qa.json', 'w') as f:
        json.dump({
